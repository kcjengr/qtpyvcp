...
class GCodeEditor : public QPlainTextEdit
{
    ...
    Q_PROPERTY(bool lazyHighlightingEnabled READ isLazyHighlightingEnabled WRITE setLazyHighlightingEnabled)
...
public:
    ...
    void loadFileInChunks(const QString &path);
    void setMaximumFileSizeForHighlighting(qint64 bytes);
    qint64 maximumFileSizeForHighlighting() const;
...
private slots:
    ...
    void delayedUpdateVisibleBlocks();
...
private:
    ...
    QTimer *highlightUpdateTimer;
    int lastVisibleStartBlock;
    int lastVisibleEndBlock;
    qint64 maxFileSizeForHighlighting;
    bool largeFileWarningShown;

    void applyExtraSelections();
    void updateLineNumberAreaWidth(int newBlockCount);
    void highlightCurrentLine();
    void updateLineNumberArea(const QRect &rect, int dy);
    void updateVisibleBlockRange();
    bool isFileSizeExceedsLimit(const QString &path) const;
