...
class TestUserManagementSetWidgetEnablement:
    def test_set_widget_enablement_checks_security_attr(self):
        ...
        import qtpyvcp.plugins.user_managment as um_mod
        orig = um_mod.qApp
        try:
            mock_qapp = MagicMock()
            mock_qapp.allWidgets.return_value = [mock_widget]
            um_mod.qApp = mock_qapp
            um.currentUserLevel.value = 10
    ...
    def test_set_widget_enablement_skips_widgets_without_setEnabled(self):
        ...
        try:
            mock_qapp = MagicMock()
            mock_qapp.allWidgets.return_value = [widget]
            um_mod.qApp = mock_qapp
            um = UserManagement()
            um.setWidgetEnablementPerPermission(None)
        finally:
    ...
    def test_set_widget_enablement_with_multiple_widgets(self):
        ...
        try:
            mock_qapp = MagicMock()
            mock_qapp.allWidgets.return_value = [w1, w2]
            um_mod.qApp = mock_qapp
            um = UserManagement()
