...
class VTKBackPlot(QVTKRenderWindowInteractor, VCPWidget, BaseBackPlot):
    ...
    def load_program(self, fname=None):
        ...
        for wcs_index, actor in self.path_actors.items():
            ...
            if len(self.path_actors) > 1:  # Draw rapids between paths when using multiple offsets
                
                # Load the start point of rapid from the next offset paths
                point_01_pos = self.path_offset_start_point.get(wcs_index)
                
                if point_01_pos is not None:
                    
                    p1_points = vtk.vtkPoints()
                    p1_vertices = vtk.vtkCellArray()
                    
                    point_01_id = [0]
                    point_01_id[0] = p1_points.InsertNextPoint(point_01_pos)
                    p1_vertices.InsertNextCell(1, point_01_id)
    
                    p1_point = vtk.vtkPolyData()
                    p1_point.SetPoints(p1_points)
                    p1_point.SetVerts(p1_vertices)
    
                    mapper = vtk.vtkPolyDataMapper()
                    mapper.SetInputData(p1_point)
                ...
                if point_02_pos is not None:
                    ...
                    p2_points = vtk.vtkPoints()
                    p2_vertices = vtk.vtkCellArray()
                
                    point_02_id = [0]
                    point_02_id[0] = p2_points.InsertNextPoint(point_02_pos)
                    p2_vertices.InsertNextCell(1, point_02_id)

                    point = vtk.vtkPolyData()
                    point.SetPoints(p2_points)
                    point.SetVerts(p2_vertices)
    ...
    def rotate_and_translate(self):
        ...
        # Get the WCS index mapping from canon
        wcs_list = self.canon.get_offsets_list()
        ...
        for i, wcs_index in enumerate(wcs_indices):
            ...
            # Only try to transform actors that exist
            start_actor = self.offset_change_start_actor.get(wcs_index)
            end_actor = self.offset_change_end_actor.get(wcs_index)

            if start_actor:
                start_actor.SetUserTransform(actor_transform)

            if end_actor:
                end_actor.SetUserTransform(actor_transform)

            # Draw line for current WCS index
            if i > 0:  # Not the first WCS
                self.draw_wcs_transition_line(prev_wcs_index, wcs_index)

            # Draw line for the last WCS index
            if i == len(wcs_indices) - 1:
                self.draw_wcs_transition_line(wcs_index, None)
    ...
    def draw_wcs_transition_line(self, start_wcs_index, end_wcs_index):
        point_01 = self.offset_change_end_actor.get(start_wcs_index)
        point_02 = self.offset_change_start_actor.get(end_wcs_index) if end_wcs_index is not None else None

        if point_01 and (point_02 or end_wcs_index is None):
            point_01_pos = point_01.GetCenter()
            point_02_pos = point_02.GetCenter() if point_02 else self.path_offset_end_point.get(start_wcs_index)

            if point_02_pos:
                actor_p01_pos = list(point_01_pos)
                actor_p02_pos = list(point_02_pos)
                actor_p03_pos = [point_02_pos[0], point_02_pos[1], point_01_pos[2]]
                ...
                self.offset_change_line_actor[start_wcs_index] = actor_line
                self.renderer.AddActor(actor_line)
