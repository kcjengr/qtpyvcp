...
class LinuxCncDataSource(QObject):
    ...
    def _configure_vtk_machine_axes(self):
        ...
        if missing_axes or invalid_axes or extra_axes:
            details = [
                "machine axes={}".format(active_axes),
                "vtk axes={}".format(configured_axes),
            ]
            if missing_axes:
                details.append("missing={}".format(missing_axes))
            if extra_axes:
                details.append("extra={}".format(extra_axes))
            if invalid_axes:
                details.append("invalid={}".format(invalid_axes))
        ...
        if not os.path.isfile(machine_parts_path):
            self._warn_vtk(
                "MACHINE_PARTS file not found: {}".format(machine_parts_path)
            )
            return
        ...
        try:
            ...
            self._warn_vtk(
                "MACHINE_PARTS YAML parse failed for {}: {}".format(machine_parts_path, exc)
            )
            return
        except OSError as exc:
            self._warn_vtk(
                "MACHINE_PARTS file read failed for {}: {}".format(machine_parts_path, exc)
            )
            return

        if not isinstance(machine_parts_data, dict):
            self._warn_vtk(
                "MACHINE_PARTS did not parse to a mapping: {}".format(machine_parts_path)
            )
            return
        ...
        if missing_from_yaml:
            ...
            self._warn_vtk(
                "MACHINE_PARTS is missing active machine axes: {}".format(missing_from_yaml)
            )
        ...
        for axis in ('A', 'B', 'C'):
            ...
            if axis_type != 'angular':
                self._warn_vtk(
                    "MACHINE_PARTS axis {} should be type=angular for table rotary ownership (got type={})".format(
                        axis,
                        axis_type or 'unset',
                    )
                )
        ...
        if missing_rotary_origins:
            self._warn_vtk(
                "table rotary axes missing MACHINE_PARTS origin definitions: {}".format(missing_rotary_origins)
            )

        legacy_pivot_keys = []
        for axis in ('A', 'B', 'C'):
            for key_suffix in ('PIVOT', 'ORIGIN', 'ROT_CENTER', 'PIVOT_X', 'PIVOT_Y', 'PIVOT_Z'):
                key = "{}_{}".format(axis, key_suffix)
                if self._inifile.find("VTK", key) is not None:
                    legacy_pivot_keys.append(key)

        if legacy_pivot_keys:
            self._warn_vtk(
                "legacy VTK pivot keys are ignored; use MACHINE_PARTS origins instead (keys={})".format(legacy_pivot_keys)
            )
